...
var RTCMultiConnection = function (roomid, forceOptions) {
    function SocketConnection(connection, connectCallback) {
        ...
        function onMessageEvent(message) {
            if (message.remoteUserId == connection.userid) {
                ...
                return message.message.changedUUID && connection.peers[message.message.oldUUID] && (connection.peers[message.message.newUUID] = connection.peers[message.message.oldUUID],
        ...
        connection.peers = {
            ...
            this.replaceTrack = function (track, remoteUserId, isVideoTrack) {
                ...
                return peer.getSenders && "function" == typeof peer.getSenders && peer.getSenders().length ? void peer.getSenders().forEach(function (rtpSender) {
                    isVideoTrack && "video" === rtpSender.track.kind && (connection.peers[remoteUserId].peer.lastVideoTrack = rtpSender.track,
                        rtpSender.replaceTrack(track)),
                        isVideoTrack || "audio" !== rtpSender.track.kind || (connection.peers[remoteUserId].peer.lastAudioTrack = rtpSender.track,
                            rtpSender.replaceTrack(track))
    ...
    function isUnifiedPlanSuppored() {
        ...
        try {
            ...
            try {
                ...
                isUnifiedPlanSupported = "unified-plan" == config.sdpSemantics || ("plan-b" == config.sdpSemantics,
                    !1)
            } catch (e) {
    ...
    function PeerInitiator(config) {
        ...
        if (connection.attachStreams.forEach(function (stream) {
            ...
            try {
                ...
                "undefined" != typeof connection.iceCandidatePoolSize && (params.iceCandidatePoolSize = connection.iceCandidatePoolSize),
                    ...
                    connection.iceServers && connection.iceServers.length || (params = null,
        ...
        !peer.getRemoteStreams && peer.getReceivers && (peer.getRemoteStreams = function () {
            ...
            localStreams.forEach(function (localStream) {
                config.remoteSdp && config.remoteSdp.remotePeerSdpConstraints && config.remoteSdp.remotePeerSdpConstraints.dontGetRemoteStream || config.dontAttachLocalStream || (localStream = connection.beforeAddingStream(localStream, self),
        ...
        peer.ontrack = function (event) {
            if (event && "track" === event.type) {
                ...
                streamToShare ? (event.stream.isAudio = streamToShare.isAudio,
                    ...
                    event.stream.isScreen = streamToShare.isScreen) : (event.stream.isVideo = !!getTracks(event.stream, "video").length,
    ...
    function onMessageCallback(data) {
        ...
        "rtcmulticonnection-extension-loaded" == data && (chromeMediaSource = "desktop"),
            data.sourceId && screenCallback && screenCallback(sourceId = data.sourceId, data.canRequestAudioTrack === !0)
    }
    ...
    function getSourceId(callback) {
        ...
        return sourceId ? callback(sourceId) : (screenCallback = callback,
    ...
    function getChromeExtensionStatus(extensionid, callback) {
        ...
        2 != arguments.length && (callback = extensionid,
    ...
    function getScreenConstraints(callback, captureSourceIdWithAudio) {
        ...
        return "desktop" != chromeMediaSource || sourceId ? ("desktop" == chromeMediaSource && (screen_constraints.mandatory.chromeMediaSourceId = sourceId),
            void callback(null, screen_constraints)) : void (captureSourceIdWithAudio ? getSourceIdWithAudio(function (sourceId, canRequestAudioTrack) {
                screen_constraints.mandatory.chromeMediaSourceId = sourceId,
                    canRequestAudioTrack && (screen_constraints.canRequestAudioTrack = !0),
    ...
    !function (that) {
        that && "undefined" == typeof window && "undefined" != typeof global && (global.navigator = {
            ...
            "undefined" == typeof global.console.debug && (global.console.debug = global.console.info = global.console.error = global.console.log = global.console.log || function () {
        ...
        function () {
            function getBrowserInfo() {
                ...
                if (isSafari && !isChrome && nAgt.indexOf("CriOS") !== -1 && (isSafari = !1,
                    isChrome = !0),
                    isOpera) {
                    browserName = "Opera";
                    try {
                    ...
                    isIE ? (verOffset = nAgt.indexOf("rv:"),
                        verOffset > 0 ? fullVersion = nAgt.substring(verOffset + 3) : (verOffset = nAgt.indexOf("MSIE"),
                            fullVersion = nAgt.substring(verOffset + 5)),
                        browserName = "IE") : isChrome ? (verOffset = nAgt.indexOf("Chrome"),
                            browserName = "Chrome",
                            fullVersion = nAgt.substring(verOffset + 7)) : isSafari ? (verOffset = nAgt.indexOf("Safari"),
                                ...
                                navigator.userAgent.indexOf("Version/") !== -1 && (fullVersion = navigator.userAgent.split("Version/")[1].split(" ")[0])) : isFirefox ? (verOffset = nAgt.indexOf("Firefox"),
                                    browserName = "Firefox",
            ...
            function DetectLocalIPAddress(callback, stream) {
                if (DetectRTC.isWebRTCSupported) {
                    var isPublic = !0
                        , isIpv4 = !0;
                    getIPs(function (ip) {
                        ip ? ip.match(regexIpv4Local) ? (isPublic = !1,
                            callback("Local: " + ip, isPublic, isIpv4)) : ip.match(regexIpv6) ? (isIpv4 = !1,
                                callback("Public: " + ip, isPublic, isIpv4)) : callback("Public: " + ip, isPublic, isIpv4) : callback()
                    }, stream)
                }
            }
            function getIPs(callback, stream) {
                ...
                if ("undefined" != typeof document && "function" == typeof document.getElementById) {
                    ...
                    if (RTCPeerConnection) {
                        ...
                        if (stream && (pc.addStream ? pc.addStream(stream) : pc.addTrack && stream.getTracks()[0] && pc.addTrack(stream.getTracks()[0], stream)),
            ...
            function checkDeviceSupport(callback) {
                ...
                navigator.enumerateDevices(function (devices) {
                    devices.forEach(function (_device) {
                        ...
                        alreadyUsedDevices[device.deviceId + device.label + device.kind] || ("audio" === device.kind && (device.kind = "audioinput"),
                            ...
                            device.label ? ("videoinput" !== device.kind || isWebsiteHasWebcamPermissions || (isWebsiteHasWebcamPermissions = !0),
                                "audioinput" !== device.kind || isWebsiteHasMicrophonePermissions || (isWebsiteHasMicrophonePermissions = !0)) : (device.isCustomLabel = !0,
                                    "videoinput" === device.kind ? device.label = "Camera " + (videoInputDevices.length + 1) : "audioinput" === device.kind ? device.label = "Microphone " + (audioInputDevices.length + 1) : "audiooutput" === device.kind ? device.label = "Speaker " + (audioOutputDevices.length + 1) : device.label = "Please invoke getUserMedia once.",
            ...
            "undefined" == typeof MediaStream && "undefined" != typeof webkitMediaStream && (MediaStream = webkitMediaStream),
                "undefined" != typeof MediaStream && "function" == typeof MediaStream ? DetectRTC.MediaStream = Object.keys(MediaStream.prototype) : DetectRTC.MediaStream = !1,
    ...
    var MediaStream = window.MediaStream;
    "undefined" == typeof MediaStream && "undefined" != typeof webkitMediaStream && (MediaStream = webkitMediaStream),
        "undefined" != typeof MediaStream && ("stop" in MediaStream.prototype || (MediaStream.prototype.stop = function () {
        ...
        , CodecsHandler = function () {
            ...
            function splitLines(sdp) {
                ...
                return sdp.split("\n").forEach(function (line) {
                    0 === line.indexOf("m=video") && (info.videoCodecNumbers = [],
                        line.split("SAVPF")[1].split(" ").forEach(function (codecNumber) {
                            codecNumber = codecNumber.trim(),
                                codecNumber && codecNumber.length && (info.videoCodecNumbers.push(codecNumber),
                        ...
                        line.indexOf("H264/90000") === -1 || info.h264LineNumber || (info.h264LineNumber = line.replace("a=rtpmap:", "").split(" ")[0])
                }),
                    info
            }
            function removeVPX(sdp) {
                var info = splitLines(sdp);
                return sdp = preferCodecHelper(sdp, "vp9", info, !0),
            ...
            if (stream && stream.addEventListener) {
                ...
                stream.mute = function (type, isSyncAction) {
                    type = handleType(type),
                        ...
                        "undefined" != typeof type && "audio" != type || getTracks(stream, "audio").forEach(function (track) {
                            track.enabled = !1,
    ...
    var globalCounter = 0;
    var sourceId, screenCallback, chromeMediaSource = "screen", isFirefox = "undefined" != typeof window.InstallTrigger, isOpera = !!window.opera || navigator.userAgent.indexOf(" OPR/") >= 0, TextSender = (!!window.chrome && !isOpera,
        {
            ...
            function handle(connection) {
                ...
                connection.onFileStart = function (file) {
                    ...
                    div.setAttribute("style","margin-left:2.5%;font-weight:500;");
                    globalCounter = globalCounter + 1;
                    div.id = 'ft_' + file.name.replace(/[\W_]/g, "") + globalCounter;
                    return div.title = file.name,
                        div.innerHTML = "<label>0%</label> <progress></progress>",
                        file.remoteUserId && (div.innerHTML += " (Sharing by:" + file.extra['senderId'] + ")"),
                    ...
                    connection.onFileEnd = function (file) {
                        ...
                        var firstNameUpperCase = null;
                        var senderNameArray = setAttendeeName(file.extra['senderId']);
                        ...
                        if (!file.remoteUserId || (helper = progressHelper[file.uuid][file.remoteUserId])) {
                            ...
                            html = '<p><a style="color:#000;" href="' + file.url + '" download="' + file.name + '">' +
                                '<strong>' + file.name + '</strong><span><i class="fa fa-download" style="color: #43A047; margin-left:10px;" aria-hidden="true"></i></span> </a></p>';
                            div.className = 'chat-background-invitee';
                            html += '<span style="color: #7d7d7f">' + formatDateRTC(new Date()) + '</span>' + ' <span class="time-left">' + firstNameUpperCase + '</span>';
                            div.innerHTML = html;
                            if (document.querySelectorAll('[title="' + file.name + '"').length > 1 && file.extra['senderId'] == localStorage["loggedInuserName"]) {

                                if (div.id != 'ft_' + file.name.replace(/[\W_]/g, "") + "1") {
                                    div.setAttribute("style", "display:none");
                                }
                                else {
                                    var fileCounter = document.getElementById('fileCount').innerHTML;
                                    fileCounter++;
                                    var fileCounterEl = document.getElementById('fileCount');
                                    fileCounterEl.innerHTML = fileCounter;
                                    globalCounter = 0;
                                }
    ...
    !function (connection) {
        ...
        function joinRoom(connectionDescription, cb) {
            connection.socket.emit("join-room", {
                ...
                if (isRoomJoined === !0) {
                    if (connection.enableLogs && console.log("isRoomJoined: ", isRoomJoined, " roomid: ", connection.sessionid),
        ...
        function openRoom(callback) {
            connection.enableLogs && console.log("Sending open-room signal to socket.io"),
                ...
                connection.socket.emit("open-room", {
                    ...
                    isRoomOpened === !0 && (connection.enableLogs && console.log("isRoomOpened: ", isRoomOpened, " roomid: ", connection.sessionid),
                        callback(isRoomOpened, connection.sessionid)),
                        isRoomOpened === !1 && (connection.enableLogs && console.warn("isRoomOpened: ", error, " roomid: ", connection.sessionid),
            ...
            return session.oneway && "two-way" !== session.audio && "two-way" !== session.video && "two-way" !== session.screen ? void callback() : (session.oneway && session.audio && "two-way" === session.audio && (session = {
                ...
                void ((session.audio || session.video || session.screen) && (session.screen ? "Edge" === DetectRTC.browser.name ? navigator.getDisplayMedia({
                    ...
                    screen.isScreen = !0,
                        ...
                        !session.audio && !session.video || isAudioPlusTab(connection) ? callback(screen) : connection.invokeGetUserMedia(null, callback)
                }, function (error) {
        ...
        mPeer.onGettingLocalMedia = function (stream, callback) {
            ...
            connection.openOrJoin = function (roomid, callback) {
                callback = callback || function () { }
                    ...
                    connection.checkPresence(roomid, function (isRoomExist, roomid) {
                        ...
                        return connection.waitingForLocalMedia = !0,
                            connection.isInitiator = !0,
                            connection.sessionid = roomid || connection.sessionid,
                            isData(connection.session) ? void openRoom(callback) : void connection.captureUserMedia(function () {
                                openRoom(callback)
                            })
                    })
            }
            ...
            connection.processSdp = function (sdp) {
                return isUnifiedPlanSupportedDefault() ? sdp : "Safari" === DetectRTC.browser.name ? sdp : ("VP8" === connection.codecs.video.toUpperCase() && (sdp = CodecsHandler.preferCodec(sdp, "vp8")),
                    ...
                    "Firefox" === DetectRTC.browser.name ? sdp : ((connection.bandwidth.video || connection.bandwidth.screen) && (sdp = CodecsHandler.setApplicationSpecificBandwidth(sdp, connection.bandwidth, !!connection.session.screen)),
            ...
            connection.addStream = function (session, remoteUserId) {
                ...
                return session.getTracks ? (connection.attachStreams.indexOf(session) === -1 && (session.streamid || (session.streamid = session.id),
                    connection.attachStreams.push(session)),
                    void connection.renegotiate(remoteUserId)) : isData(session) ? void connection.renegotiate(remoteUserId) : void ((session.audio || session.video || session.screen) && (session.screen ? "Edge" === DetectRTC.browser.name ? navigator.getDisplayMedia({
                        video: !0,
                        audio: isAudioPlusTab(connection)
                    }).then(function (screen) {
                        screen.isScreen = !0,
                            mPeer.onGettingLocalMedia(screen),
                            !session.audio && !session.video || isAudioPlusTab(connection) ? gumCallback(screen) : connection.invokeGetUserMedia(null, function (stream) {
                                gumCallback(stream)
                            })
                    }, function (error) {
                        console.error("Unable to capture screen on Edge. HTTPs and version 17+ is required.")
                    ...
                    getUserMediaHandler({
                        onGettingLocalMedia: function (stream) {
                            ...
                            videoConstraints && (videoConstraints.mediaSource || videoConstraints.mozMediaSource ? stream.isScreen = !0 : videoConstraints.mandatory && videoConstraints.mandatory.chromeMediaSource && (stream.isScreen = !0)),
                                stream.isScreen || (stream.isVideo = !!getTracks(stream, "video").length,
            ...
            connection.replaceTrack = function (session, remoteUserId, isVideoTrack) {
                ...
                if (session instanceof MediaStream)
                    return getTracks(session, "video").length && replaceTrack(getTracks(session, "video")[0], remoteUserId, !0),
                ...
                (session.audio || session.video || session.screen) && (session.screen ? "Edge" === DetectRTC.browser.name ? navigator.getDisplayMedia({
                    ...
                    screen.isScreen = !0,
                        mPeer.onGettingLocalMedia(screen),
                        !session.audio && !session.video || isAudioPlusTab(connection) ? gumCallback(screen) : connection.invokeGetUserMedia(null, gumCallback)
                }, function (error) {
                    console.error("Unable to capture screen on Edge. HTTPs and version 17+ is required.")
                }) : connection.getScreenConstraints(function (error, screen_constraints) {
            ...
            connection.getSocket = function (callback) {
                return !callback && connection.enableLogs && console.warn("getSocket.callback paramter is required."),
                    callback = callback || function () { }
                    ,
        ...
        if (connection.streamEvents = {
            ...
            connection.checkPresence = function (roomid, callback) {
                return roomid = roomid || connection.sessionid,
                    "SSEConnection" === SocketConnection.name ? void SSEConnection.checkPresence(roomid, function (isRoomExist, _roomid, extra) {
                        return connection.socket ? void callback(isRoomExist, _roomid) : (isRoomExist || (connection.userid = _roomid),
        ...
        connection.getExtraData = function (remoteUserId, callback) {
            ...
            connection.setPassword = function (password, callback) {
                callback = callback || function () { }
                    ,
                    connection.socket ? connection.socket.emit("set-password", password, callback) : (connection.password = password,
...
"undefined" != typeof module && (module.exports = exports = RTCMultiConnection),
    "function" == typeof define && define.amd && define("RTCMultiConnection", [], function () {
